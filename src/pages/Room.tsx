...
import message_notification from "../assets/sounds/message.mp3";
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      if (!showChat && msgs.length > messages.length) {
        const newMessageCount = msgs.length - messages.length;
        setUnreadCount((prev) => prev + newMessageCount);

        // Notification sesi çal (muted kapalıysa)
        if (!muted) {
          try {
            const audio = new Audio(message_notification);
            audio.volume = 0.5;
            audio
              .play()
              .catch((err) => console.log("Notification sound could not play"));
          } catch (error) {
    ...
    return () => unsub();
  }, [roomId, showChat, messages.length, muted]);
